// ─────────────────────────────────────────────────────────────
//  오른팔 보드 (R_UPPERARM / R_FOREARM) — 순수 6축 + 동적 자이로 캘리브레이션
...
#define BOARD_NAME  "ARM_BOARD_R"
#define FW_VERSION  "arm-r-6ax-1.4"
...
void setup() {
  ...
  Serial.println(">>> [오른팔 순수 6축 v1.4] 준비 완료 <<<");
  Serial.println(CMD_HELP);
}
...
void loop() {
  ...
  if (btnPressed && webSocket.isConnected()) {
    lastBtnTime = now;
    webSocket.sendTXT("{\"cmd\":\"zero\",\"arm\":\"L\"}");
    Serial.println(">>> 오른팔 영점 명령 서버로 전송 <<<");
  }
