...
// ⭐⭐ 센서 ID
//   ⚠ 왼팔 보드는 배치가 아직 확인되지 않았습니다.
//     오른팔 보드는 실측 결과 0x68=전완, 0x69=상완 이었지만
//     왼팔은 배선/부착이 다를 수 있으니 반드시 확인하세요.
//
//   확인법: 업로드 후 시리얼 모니터를 열고 "팔꿈치만" 굽혔다 폈다 해보세요.
//           L_UPPERARM 값이 거의 안 변하고 L_FOREARM 만 크게 변하면 정상입니다.
//           반대로 나오면 아래 두 줄을 서로 맞바꾸세요.
#define SENSOR_ID_1 "L_FOREARM"    // MPU1(0x68)
#define SENSOR_ID_2 "L_UPPERARM"   // MPU2(0x69)
...
// ⭐ 왼팔 보드 오프셋 (가속도/자이로) - mpu_calibration.ino 실측값
const int16_t MPU1_AX_OFFSET = 849;
const int16_t MPU1_AY_OFFSET = 471;
const int16_t MPU1_AZ_OFFSET = -1163;
float MPU1_GX_OFFSET = -71.7;
float MPU1_GY_OFFSET = 66.1;
float MPU1_GZ_OFFSET = 112.9;

const int16_t MPU2_AX_OFFSET = -805;
const int16_t MPU2_AY_OFFSET = -219;
const int16_t MPU2_AZ_OFFSET = -3681;
float MPU2_GX_OFFSET = 160.8;
float MPU2_GY_OFFSET = 85.2;
float MPU2_GZ_OFFSET = -24.8;

// ⭐⭐⭐ 지자계 hard-iron 오프셋 (8자 캘리브레이션 측정값, raw 단위)
//   9축은 상완=MPU2(0x69) 에만 적용한다. (0x0C 주소 충돌 때문에 한쪽만 가능)
//
//   ⚠⚠ 아직 측정 전입니다. 지금 상태로 업로드하면 자동으로 6축으로 동작합니다.
//       mag_calibration.ino 를 이 보드에 올려 30초간 8자로 돌린 뒤,
//       출력된 MPU2 두 줄을 아래에 그대로 덮어쓰세요.
//   ⚠⚠ 오른팔 값을 복사하면 절대 안 됩니다. 보드마다 완전히 다른 값입니다.
float MPU1_MX_OFFSET = 0.0, MPU1_MY_OFFSET = 0.0, MPU1_MZ_OFFSET = 0.0;
float MPU2_MX_OFFSET = 49.5, MPU2_MY_OFFSET = 86.0, MPU2_MZ_OFFSET = -177.0;
// soft-iron 스케일
float MPU1_MX_SCALE = 1.0, MPU1_MY_SCALE = 1.0, MPU1_MZ_SCALE = 1.0;
float MPU2_MX_SCALE = 0.975, MPU2_MY_SCALE = 1.155, MPU2_MZ_SCALE = 0.902;
...
struct MagCal {
  ...
  // hard-iron 보정값이 0 이면 false 로 두어 6축으로 떨어뜨린다.
  // 보정 안 된 지자계는 yaw 를 통째로 틀어버려서 6축보다 나쁘다.
  bool ready = false;
};
...
void setup() {
  ...
  // 지자계 초기화: 상완인 MPU2(0x69)에 9축 적용
  setupMag(MPU2_ADDR, mag2cal);
  if (MPU2_MX_OFFSET == 0.0 && MPU2_MY_OFFSET == 0.0 && MPU2_MZ_OFFSET == 0.0) {
    mag2cal.ready = false;
    Serial.println(">>> [경고] 지자계 hard-iron 보정값이 0 입니다. 6축으로 동작합니다.");
    Serial.println(">>>        mag_calibration.ino 로 측정해서 MPU2_M*_OFFSET 에 넣으세요.");
  } else {
    mag2cal.ready = true;
    Serial.print(">>> 지자계는 ");
    Serial.print(SENSOR_ID_2);
    Serial.print("(MPU2=0x69)만 사용. ");
    Serial.print(SENSOR_ID_1);
    Serial.println("(MPU1=0x68)은 6축 <<<");
  }
