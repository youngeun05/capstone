// ─────────────────────────────────────────────────────────────
//  왼팔 보드 (L_UPPERARM / L_FOREARM) — 순수 6축 + 동적 자이로 캘리브레이션
...
// ⭐⭐ 센서 ID (0x68 = 전완, 0x69 = 상완)
#define BOARD_NAME  "ARM_BOARD_L"
#define FW_VERSION  "arm-l-6ax-1.4"
#define SENSOR_ID_1 "L_FOREARM"    // MPU1(0x68) = 전완
#define SENSOR_ID_2 "L_UPPERARM"   // MPU2(0x69) = 상완
...
// ⭐ 왼팔 보드 가속도 오프셋
//    ※ 아래 값은 오른팔 보드에서 실측한 값을 그대로 옮겨둔 것입니다.
//      가속도 오프셋은 개체마다 다르므로 왼팔 보드로 다시 실측해서
//      덮어써야 정확합니다 (자이로는 setup에서 자동 측정되므로 그대로 두면 됩니다).
...
void runCmd(String c) {
  ...
  if (c == "check") {
    ...
    if (webSocket.isConnected()) {
      webSocket.sendTXT("{\"cmd\":\"zero\",\"arm\":\"L\"}");
      Serial.println("\n[zero] 서버로 왼팔 영점 명령 전송");
    } else {
...
void setup() {
  ...
  Wire.setClock(400000); // 팔은 배선이 짧아 400kHz
  ...
  Serial.println(">>> [왼팔 순수 6축 v1.4] 준비 완료 <<<");
  Serial.println(CMD_HELP);
}
